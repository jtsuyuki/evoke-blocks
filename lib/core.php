...
abstract class Core {
	...
	/**
	 * takes an array of types and initializes the internal $types array so
	 * it's indexed by type_name and contains the type meta data as its value
	 *
	 * @param $types
	 */
	...
	/**
	 * This function generates the Custom Post Types based on the internal $types array
	 *
	 */
	...
	public static function metabox_init( $object ) {
		foreach ( self::$types as $type ) {
			add_meta_box( $type['meta_box_key'], $type['cpt_text'], function ( $object ) use ( $type ) {
				self::cpt_meta_box_markup( $object, $type['type'] );
			}, [ "page" ], "side", "high", null );
		}
	}
	...
	public static function display_by_type_page_and_number( $type, $page_id, $number ) {
		$cpt = self::get_by_type_page_and_number( $type, $page_id, $number );
		if ( $cpt ) {
			self::display_by_cpt( $cpt );
		} else {
	...
	public static function display_or_default($cpt, $default_cpt) {
		if ($cpt) {
			self::display_by_cpt($cpt);
		} else {
			self::display_by_cpt($default_cpt);
		}
	}
	...
	public static function get_by_type_page_and_number( $type, $page_id, $number ) {
		$cpts_on_page = self::get_by_type_and_page( $type, $page_id );
		if ( $cpts_on_page ) {
			$number --;
			if ( $cpts_on_page[ $number ] ) {
				return get_post( $cpts_on_page[ $number ]);
			} else {
				return null;
			}
		} else {
